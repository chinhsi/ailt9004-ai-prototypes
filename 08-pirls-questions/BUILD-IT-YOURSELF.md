...
Requirements:
- Settings panel: API key (password field, saved in localStorage), base URL (default https://openrouter.ai/api/v1), comma-separated model list (default "inclusionai/ling-3.0-flash-fin:free, nvidia/nemotron-3-super-120b-a12b:free"). Keys never leave localStorage.
...
- Render each item as a card colour-coded by process, with badges for process, format and marks, the options as A-D, and a teacher block holding answer, scoring guide and the why line. Normalise the model's answer field: accept "B", "B." or the option text, and show letter + option text.
...
- Parse JSON leniently (strip code fences, take the outermost braces); on 429/500/502/503 wait 1.5 s, retry once, then fall through to the next model; 90 s timeout; stop trying a model that returns 402/403. Plain CSS, no libraries, works from file://.
