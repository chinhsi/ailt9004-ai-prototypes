...
- **Before changing `DEFAULT_PROMPT` or `drawCard()`, read the handover note** at `~/Google Drive/clresearch/clresearch/01-COURSES/AILT9004-Interactive-AI/Shared/Irony-Mirror-04-進度與交接.md`. Two things there are load-bearing and were each paid for with a live test run:
  - The `poem` field carries four stanzas in a fixed rhetorical form (mock definition / antonym couplet / parallel clauses with a turn / ironic compliment). The counter-examples written into the spec are there because the model produced exactly those failures — do not tidy them away as clutter.
  - The card is a 720×1080 portrait poster whose lines are wrapped by measuring them with canvas `measureText` in the same font the SVG draws with. Do not replace that with a character count: that is what used to push text past the border.
- This prototype currently exists in two places — this repository and `~/Google Drive/HKU/Courses/2026 09/AILT9004/Materials/Prototypes/04-irony-mirror/`. Diff them before editing; ask the user which side is current if they differ.
- To verify: paste the key, click the chip 孝順, wait: two captions and *The gap* should appear within ~10 s; pictures within ~60 s. The poster should show four stanzas with no line crossing the margin. Then open *Game*, press *Start*, click a picture, press *Reveal*. Layout-only changes can be checked without a key using the Playwright scripts in `Materials/Prototypes/_dev/04-irony-mirror/`.
