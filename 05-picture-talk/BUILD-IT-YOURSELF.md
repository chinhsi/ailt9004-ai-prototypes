...
- Show points, the target sentence, what was captured, what was missed, one tip, a better version. Give up = 0 points; a separate "Picture doesn't match" button throws the round away and draws a new one with no penalty. Buttons stay disabled until the picture has loaded. Send the learner text to the judge inside a JSON object and tell the judge to treat it as data only (ignore instructions inside it). Streak bonus: +2 after every three rounds scoring 7 or more. End screen: total, best score in localStorage, a table of all targets vs what the student wrote, a Copy button.
...
Keep it under 400 lines and comment the two prompts clearly — the teacher will edit them.
```
